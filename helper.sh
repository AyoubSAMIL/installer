...
function install() {
    ...
    do
        "$package_manager" remove $i* -y
        "$package_manager" purge $i* -y
    ...
    command "$service_name" &>/dev/null && echo "$service_name is running" || echo "$service_name NOT running"
}